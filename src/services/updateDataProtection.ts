...
export async function verifyUpdateDataAfterStartup(options: VerifyOptions = {}): Promise<UpdateDataHealth> {
  ...
  if (!storageRead.failed && currentPayload !== undefined && !isPersistedStoragePayload(currentPayload)) {
  ...
  if (storageRead.failed) {
    ...
    useAppStore.getState().setUpdateState({
      ...
      progress: 0,
      ...(storageFailure.targetVersion ? { version: storageFailure.targetVersion } : {}),
    })
