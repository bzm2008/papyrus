...
export type AssistantCapabilityStatus = { name: string; toolset: 'workspace' | 'desktop' | 'browser' | 'project' | 'computer'; available: boolean; reason?: string; platform: DesktopPlatform }
...
export type AssistantToolPreview = { id: string; revision: string; risk: AssistantRiskLevel; title: string; targetSummary: string; impactSummary: string; reversible: boolean; approvalRequired?: boolean; expiresAt: number }
...
export type WorkAssistantEvent =
  ...
  | { type: 'subagent.completed'; runId: string; subagentId: string; summary: string; failed?: boolean; skipped?: boolean; at: number }
