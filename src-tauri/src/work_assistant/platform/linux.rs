...
pub(crate) fn remove_empty_recovery_slot(
    ...
    let leaf = CString::new(binding.slot_leaf.as_str())
        .map_err(|_| WorkAssistantError::stale_preview("private recovery slot is invalid"))?;
    // Verify identity before unlinking and remove relative to the rebound private vault.
...
/// Re-walk a destination parent from its authorized root immediately before mutation.  Holding a
/// directory FD alone is insufficient on POSIX because an attacker can rename that directory out
/// of the workspace while the FD stays live.
pub(crate) fn verify_bound_destination(
    ...
    // Reopening the original canonical pathname is required before every mutation.  A retained
    // FD alone remains valid after its directory is moved outside the approved workspace.
    // This cannot revoke a same-identity actor racing after the rebind; the identity comparison
...
fn open_root(root: &Path) -> Result<File, WorkAssistantError> {
    ...
    ensure_private_handle_is_directory(&file)?;
    Ok(file)
}
...
fn ensure_private_handle_is_directory(file: &File) -> Result<(), WorkAssistantError> {
    if !file
        .metadata()
...
mod tests {
    ...
    fn receipt_probe_sync_failure_is_recoverable_and_leaves_no_file() {
        let root =
            std::env::temp_dir().join(format!("papyrus-linux-probe-{}", uuid::Uuid::new_v4()));
