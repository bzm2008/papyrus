...
原生代理只接受结构化的 `file_plan_batch` 协议。它不会执行模型生成的命令、脚本或任意绝对路径，也不会把回收站、文件管理器或其他桌面程序当作绕过审批的后门。每次写操作都要经过路径策略、身份快照、恢复位置预检和用户审批。
...
三个桌面平台都执行同一组不变量：拒绝链接和重解析点，比较授权根、父目录和源文件身份，保存文件版本与内容摘要，并在提交前重新验证快照。写入先进入同目录临时文件，再通过平台的 no-replace 原语发布；覆盖会先把旧目标移入恢复库。任何身份、路径组件、共享模式或恢复预检不匹配都会 fail closed。
...
跨设备移动只在能先复制、校验发布结果，再把原文件移入同卷恢复库时继续。不能建立同卷恢复库、不能创建并清理 receipt 预检文件，或缺少 no-replace 原语时，能力状态必须显示不可用，执行返回可解释的 `recovery_unavailable` 或 `blocked`，不得改用未经保护的路径操作。
...
- `partial_transaction`：旧目标或原文件已经安全进入恢复库，但发布或后续步骤失败；显示可恢复警告，若结果带 `recoveryReceipts` 则展示，否则按预览 ID 引导用户检查恢复或审计入口，不能声称“没有发生任何变化”
